import 'package:flutter/material.dart';
import 'package:kostqu/Component/Function.dart';
import 'package:kostqu/Component/Style.dart';
import 'package:kostqu/Component/topBar.dart';
import 'package:kostqu/Connection/Services.dart';
import 'package:kostqu/Models/ModelsMaster.dart';
import 'package:kostqu/Models/ModelsMaster.dart';
import 'package:kostqu/UI/Consumen/ListTransport.dart';
import 'package:kostqu/UI/Owner/Master/List.dart';

class DataMaster extends StatefulWidget {
  @override
  _DataMasterState createState() => _DataMasterState();
}

class _DataMasterState extends State<DataMaster> {
  var loading = false;
  List<ModelsMaster> listdata = [];

  Future<void> getData() async {
    if (!mounted) return;
    setState(() {
      loading = true;
    });
    Services services = Services(context: context);
    List<ModelsMaster> tempData = await services.getDataMaster();

    setState(() {
      loading = false;
      listdata = tempData;
    });
  }

  @override
  void initState() {
    super.initState();
    getData();
  }

  @override
  Widget build(BuildContext context) {
    return Container(
        padding: EdgeInsets.symmetric(vertical: 5),
        child: Column(
          children: [
            Text(
              'Data Master',
              style: Style.textStyleHeader1(fontColor: Style.colorBlue),
            ),
            Expanded(
              child: ListView.builder(
                  itemCount: listdata.length,
                  itemBuilder: (context, index) {
                    return InkWell(
                      onTap: () {
                        Func().nav(context, ListMaster(listdata[index].nama));
                      },
                      child: Container(
                          decoration: BoxDecoration(
                              color: Style.colorBlue,
                              borderRadius:
                                  BorderRadius.all(Radius.circular(10))),
                          margin: EdgeInsets.all(Style.l(context) / 50),
                          padding: EdgeInsets.all(Style.l(context) / 50),
                          child: Row(
                            children: [
                              Container(
                                width: 110,
                                height: 100,
                                child: listdata[index].nama == "Kamar"
                                    ? Icon(
                                        Icons.hotel,
                                        size: 60,
                                        color: Style.colorOrange,
                                      )
                                    : Icon(
                                        Icons.directions_bike,
                                        size: 60,
                                        color: Style.colorOrange,
                                      ),
                              ),
                              SizedBox(
                                width: 5,
                              ),
                              Expanded(
                                flex: 5,
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(
                                      listdata[index].nama,
                                      style: Style.textStyleHeader3(
                                          bold: true, fontColor: Colors.white),
                                    ),
                                  ],
                                ),
                              ),
                            ],
                          )),
                    );
                  }),
            )
          ],
        ));
  }
}
